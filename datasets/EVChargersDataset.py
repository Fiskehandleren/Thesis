...
class EVChargersDataset(pl.LightningDataModule):
    def __init__(
        ...
        **kwargs
        ...
        if not os.path.exists(os.path.join(feat_path, dataset_name)):
            ...
            self._feat = dataloader.create_count_data(dataloader.load_data(), session_minutes, save=True).to_numpy()
        else:
            self._feat = pd.read_csv(os.path.join(feat_path, dataset_name)).drop('Period', axis=1).to_numpy()
    ...
    def add_data_specific_arguments(parent_parser):
        ...
        parser.add_argument("--cluster", type=int, help="Which cluster index to fit an AR model to")
